...
def calculate_green(request, node_id):
    ...
    # ---------- Validate node ----------
    # if not Node.objects(node_id=node_id):
    #     return Response({"error": "Node not found"}, status=404)

    # ---------- Uploaded images ----------
    # request.FILES is dict: { edge_id : UploadedFile }
    ...
    #     return Response(
    #         {"error": "No outgoing edges for this node"},
    #         status=400
    #     )
    ...
    for edge_id, image_file in uploaded.items():
        if edge_id not in outgoing_edges:
            print(f"[GREEN] ⚠️ Skipping invalid edge {edge_id} for node {node_id}")
            continue

        edge = outgoing_edges[edge_id]
        print("[GREEN] processing edge", edge_id, "image:", image_file.name, image_file.size)
