...
export default function MapView({
  ...
  routingNodeClickHandler  // ✅ NEW: Handler from RoutingVisualizer
}) {
  ...
  const handleNodeClick = useCallback((node) => {
    ...
    console.log('[MapView] onNodeSelect exists:', !!onNodeSelect);

    // Priority 1: Routing analyzer is active
    ...
    // Priority 2: Edge selection mode
    ...
    // Priority 3: Normal node selection
    if (onNodeSelect) {
  ...
  }, [routingNodeClickHandler, selectEdgeMode, onEdgeNodeSelect, onNodeSelect]);
    ...
    <div style={{ width: "100%", height: "100%" }}>
      ...
      >
        ...
        {!loading && nodes.map(node => {
          ...
          const signal = signalStates?.[node.node_id];
          const remaining = signal?.remainingComputed ?? signal?.remaining_time ?? 0;
          const isAlert = remaining < 10;
          
          const baseSize = 3;
          const nodeSize = Math.max(12, Math.min(24, baseSize * zoom));

          // Highlight if this is the routing start or destination node
        ...
        {!loading && nodes.map(node => {
          const signal = signalStates?.[node.node_id];
          if (!signal?.current_green) return null;

          const greenEdge = edges.find(e => e.edge_id === signal.current_green);
